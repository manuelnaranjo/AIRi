/*
 * Improved Copyright (C) 2011, Naranjo Manuel Francisco
 *
...
public class ImprovedBluetoothDevice {
    ...
    private static final Method _convertPinToBytes = getMethod(BluetoothDevice.class, "convertPinToBytes", new Class []{ String.class });
    private static final Method _createBond = getMethod(BluetoothDevice.class, "createBond", new Class [] {});
    ...
    public boolean setPin(String pin) throws Exception{
        if (_convertPinToBytes == null)
            throw new NoSuchMethodException("convertPinToBytes");
        try {
            Object pin_ = _convertPinToBytes.invoke(mDevice, pin);
            return setPin((byte[])pin_);
        } catch (InvocationTargetException tex) {
            Log.e(TAG, "", tex);
            if (tex.getCause() instanceof Exception)
                throw (Exception)tex.getCause();
    ...
    public boolean createBond() throws Exception {
        ...
        try {
            return (Boolean)_createBond.invoke(mDevice);
        } catch ( Exception e ){
            Log.e(TAG, "", e);
        }
        return false;
